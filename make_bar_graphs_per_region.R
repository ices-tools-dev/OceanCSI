...
timeperiods = c("1980-1999", paste0("2000-", assessmentYear))
...
for(timeperiod in timeperiods){
  ...
  filenames <- list.files(paste0("Output_", assessmentYear), pattern = paste0(perc, ".+", timeperiod, ".+", "dissolvedoxygen"), full.names = T)
  ...
  ggsave(
    file.path(
      ...
      paste0("barchart_", perc, "_", timeperiod, ".png")
    ), 
    width = 8, 
    height = 4
  )
  ...
  data_df %>%
    ...
    write_csv(
      file.path(
        paste0("output_", assessmentYear), 
        paste0("barchartdata_", perc, "_", timeperiod, ".csv")
      )
    )
}
