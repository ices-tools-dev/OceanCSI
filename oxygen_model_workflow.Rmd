...
stationSamples_with_new_bathymetric$Bathymetric <- stationSamples_with_new_bathymetric$newBathymetric2
...
range(stationSamples_with_new_bathymetric$Oxygen, na.rm = T)
hist(stationSamples_with_new_bathymetric$Oxygen)
...
```{r}
# run script "fixBathymetric.R" manually
```
...
```{r}
fwrite(Quartile_all, file.path(paste0("Output_", assessmentYear), paste0(prefix, "Oxygen_status.csv")))
```
...
saveEuropeStatusMap2(parameter = paste0(prefix, "Oxygen"))
```
...
make_oxygen_output <- function(
  ...
  for(cc in classes){
    ...
    if(nrow(clusterSelection != 0)){
      ...
      fwrite(
        KendallResult.clustered, 
        file.path(
          paste0("output_", assessmentYear), 
          paste0(prefix, "trend_dissolvedoxygen", cc, ".csv")
          )
        )
      ...
      plotKendallClasses(
        ...
        prefix_title = paste(prefix, cc)
      )
      saveEuropeTrendMap2(name = paste0(prefix, "Oxygen_", cc))
    }
  }
