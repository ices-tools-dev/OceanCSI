  ...
  for(cc in classes){
    ...
    if(nrow(clusterSelection != 0)){
      ...
      KendallResult.clustered <- df.KendallResult %>% 
        left_join(clusterSelection %>% distinct(SeaRegionID, ClusterID, AvgLatitude, AvgLongitude), by = c('ClusterID' = 'ClusterID')) %>%
        filter(!is.na(S)) %>%
...
```{r}
source("oassesment 2024/xygenIndicators_summary_figures_2024.R")
```
